...
if __name__ == '__main__':
    ...
    X = dataset['m5'];  Y = dataset['starch'] 
    KS_train_num = 64 

   
    CalInd, ValInd = KennardStone(X, KS_train_num)
    Xtrain = X[CalInd]; Xtest = X[ValInd]
    Ytrain = Y[CalInd]; Ytest = Y[ValInd]

        
    Xcal = Xtrain
    Ycal = Ytrain

    OUTPUT = plscvfold(Xcal, Ycal, 15, 10)
    index_A, RMSECV_ALL = OUTPUT['index_A'], OUTPUT['RMSECV_ALL']
    print(f'index_A of PLS is {index_A}')
    [X, xp1, xp2] = pretreat(Xcal)
    [y, yp1, yp2] = pretreat(Ycal)
    PLS = pls1_nipals(X, y, index_A[0])
    coef = PLS['B']
    Xcal_ypred = plspredtest(coef, Xcal, xp1, xp2, yp1, yp2)
    Xtest_ypred = plspredtest(coef, Xtest, xp1, xp2, yp1, yp2)

    
    RMSEP_cal_PLS, Q2_cal_PLS = RMSEP(Xcal_ypred, Ycal)
    RMSEP_test_PLS, Q2_test_PLS = RMSEP(Xtest_ypred , Ytest)
    
#####ELM for regress
    W = PLS['W']; P = PLS['P']
    TPLS = dot(W,inv(dot(P.T, W)))
    Tcal = dot(Xcal, TPLS)
    Ttest = dot(Xtest, TPLS)
    ...
    for i, num in enumerate(hiddens):
       ...
       pre_cal = ae.predict(Tcal)
       pre_cals.append(pre_cal)

       pre_test = ae.predict(Ttest)
       pre_tests.append(pre_test)
   
    pre_cals1 = np.asarray(pre_cals)
    pre_cal = np.mean(pre_cals1, axis=0)

    
    pre_tests1 = np.asarray(pre_tests)
    pre_test = np.mean(pre_tests1, axis=0)
    
    
    RMSEP_cal_ELM, Q2_cal_ELM = RMSEP(pre_cal, Ycal)
    RMSEP_test_ELM, Q2_test_ELM = RMSEP(pre_test, Ytest)
    
    print( "RMSEP_cal_PLS:", RMSEP_cal_PLS, "Q2_cal_PLS:", Q2_cal_PLS)
    print( "RMSEP_test_PLS:", RMSEP_test_PLS, "Q2_test_PLS:", Q2_test_PLS)   
    print( "RMSEP_cal_ELM:", RMSEP_cal_ELM, "Q2_cal_PLSELM:", Q2_cal_ELM)
    print( "RMSEP_test_ELM:", RMSEP_test_ELM, "Q2_test_PLSELM:", Q2_test_ELM)
    ...
    plt.figure()
    plt.plot(x, y)
    plt.plot(Ytest, pre_test, 'ro', label="PLSELM" )
    plt.plot(Ytest, Ytest, 'k*', label="Ytest")

    plt.xlabel("Reference values", fontsize=14)
    plt.ylabel("predicted values", fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.text(64,63,'RMSEP=%.4f'%RMSEP_test_ELM,fontsize=14) 
